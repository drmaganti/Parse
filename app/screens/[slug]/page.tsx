...
import { getPublicScreen, publicScreenFilters, PUBLIC_SCREENS } from "../../../lib/publicScreens";
...
export default function PublicScreenPage({ params }: { params: { slug: string } }) {
  const screen = getPublicScreen(params.slug);
  if (!screen) notFound();
  const savedState = encodeScreenState({ version: 1, q: screen.query, filters: publicScreenFilters(screen), ranking: screen.state.ranking });
  const tryParams = new URLSearchParams({ state: savedState, source: `public_screen:${screen.slug}` });
  const tryUrl = `/try?${tryParams.toString()}`;
  ...
  return <div style={{ minHeight: "100vh", background: T.bg, color: T.ink, fontFamily: "var(--font-body), 'Inter', system-ui, sans-serif" }}>
    ...
    <main style={{ maxWidth: 920, margin: "0 auto", padding: "52px 24px 82px" }}>
      ...
      <section style={{ marginTop: 25, maxWidth: 760 }}><div style={{ fontFamily: MONO, fontSize: 12, color: T.accent, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 11 }}>{screen.category} screen</div><h1 className="site-page-title" style={{ margin: "0 0 15px" }}>{screen.title}</h1><p style={{ color: T.inkSoft, fontSize: 16.5, lineHeight: 1.58, margin: 0 }}>{screen.summary}</p></section>

      <section style={{ marginTop: 30, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 15, padding: 22, maxWidth: 800 }}><div style={{ fontSize: 12, color: T.inkSoft, fontWeight: 650, letterSpacing: ".04em", marginBottom: 9 }}>POPULAR SCREEN</div><div style={{ fontFamily: DISP, fontSize: 22, lineHeight: 1.4, fontWeight: 500 }}>“{screen.query}”</div><div style={{ marginTop: 18, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}><a href={tryUrl} className="ps-btn">Open and edit this screen</a><span style={{ color: T.inkSoft, fontSize: 13 }}>Loads exact saved criteria without reinterpretation.</span></div></section>

      <section style={{ marginTop: 36, maxWidth: 800 }}><h2 style={{ fontFamily: DISP, fontSize: 22, margin: "0 0 13px", fontWeight: 600 }}>Exact saved criteria</h2><div style={{ display: "grid", gap: 10 }}>{screen.criteria.map((criterion, index) => <div key={criterion} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: T.surfaceAlt, border: `1px solid ${T.border}`, borderRadius: 11, padding: "13px 15px" }}><span style={{ fontFamily: MONO, color: T.accent, fontSize: 12, marginTop: 2 }}>{String(index + 1).padStart(2, "0")}</span><span style={{ fontSize: 14.5, lineHeight: 1.48 }}>{criterion}</span></div>)}</div><p style={{ color: T.inkSoft, fontSize: 13.5, lineHeight: 1.55, marginTop: 13 }}>This popular screen opens with these criteria already applied. You can edit any indicator or value to fine-tune the results.</p></section>
