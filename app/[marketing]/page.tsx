...
export default function MarketingPage({ params }: { params: { marketing: string } }) {
  ...
  return <div style={{ minHeight: "100vh", background: T.bg, color: T.ink, fontFamily: "var(--font-body), 'Inter', system-ui, sans-serif" }}>
    ...
    <header style={{ borderBottom: `1px solid ${T.border}` }}><div style={{ maxWidth: 1000, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}><ParseBrand /><div style={{ display: "flex", alignItems: "center", gap: 4 }}><a href="/screens" className="seo-btn seo-ghost seo-hide">Screen ideas</a><a href="/methodology" className="seo-btn seo-ghost seo-hide">How it works</a><a href="/try" className="seo-btn seo-primary">Try Parse</a></div></div></header>

    <main style={{ maxWidth: 1000, margin: "0 auto", padding: "58px 24px 84px" }}>
      <section style={{ maxWidth: 760 }}><div style={{ fontFamily: MONO, color: T.accent, fontSize: 12, letterSpacing: ".06em", marginBottom: 12 }}>{page.eyebrow}</div><h1 className="site-page-title" style={{ margin: "0 0 18px" }}>{page.headline}</h1><p style={{ color: T.inkSoft, fontSize: 17, lineHeight: 1.62, margin: "0 0 25px", maxWidth: 730 }}>{page.intro}</p><div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}><a href="/try" className="seo-btn seo-primary">Try a screen</a><a href="/screens" className="seo-btn seo-ghost">Browse screen ideas →</a></div><div style={{ marginTop: 12, color: T.inkSoft, fontSize: 12.5 }}>No account required for the guest experience. Research tool only—not investment advice.</div></section>
