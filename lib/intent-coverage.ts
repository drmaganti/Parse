...
const INTENT_COVERAGE: IntentCoverageSpec[] = [
  ...
  { query: /\blow debt\b|\blittle debt\b|\bminimal debt\b|\bmodest debt\b|\blow leverage\b|\blightly leveraged\b|\bleverage\s+(?:is|looks?)\s+low\b/i, coveredBy: ["debtEquity"], assumptionMentions: /low debt|little debt|minimal debt|modest debt|low leverage|lightly leveraged|leverage\s+(?:is|looks?)\s+low|debt\s*\/?\s*equity/i, message: "'Low debt/leverage' needs a debt/equity threshold; Parse left that criterion out rather than guess." },
  { query: /\breasonable valuation\b|\breasonably valued\b|\bfair valuation\b|\bfairly valued\b|\bsensible valuation\b|\bnot expensive\b|\bnot\s+too\s+(?:pricey|pricy)\b|\bnot[- ]crazy valuation(?:s)?\b/i, coveredBy: ["pe", "forwardPe", "pb", "ps", "evEbitda", "peg", "forwardPeg", "earningsYield"], assumptionMentions: /reasonable valuation|reasonably valued|fair valuation|fairly valued|sensible valuation|not expensive|not too (?:pricey|pricy)|not.?crazy valuation|valuation metric/i, message: "'Reasonable valuation' needs a specific valuation metric or threshold; Parse left that criterion out rather than guess." },
  ...
  { query: /\blow beta\b(?!\s*(?:is\s*)?(?:no more than|no less than|at most|less than|under|below|<=|<|at least|more than|greater than|over|above|>=|>)\s*-?\d)/i, coveredBy: [], assumptionMentions: /low beta|beta threshold/i, message: "'Low beta' needs an explicit beta threshold; Parse left that criterion out rather than guess." },
  { query: /\bhigh[- ]yield(?:\s+(?:stocks?|companies|names|financials?))?\b/i, coveredBy: ["divYield"], assumptionMentions: /high yield|dividend yield threshold/i, message: "'High yield' needs an explicit dividend-yield threshold; Parse left that criterion qualitative rather than inventing a cutoff." },
  { query: /\b(?:quality|high[- ]quality)\s+(?:[a-z-]+\s+){0,2}(?:stocks?|companies|names|compounders?)\b/i, coveredBy: [], assumptionMentions: /quality metric|quality screen|standalone quality|quality.*investment style/i, message: "'Quality' is an investment style rather than one supported metric; Parse left it qualitative instead of inventing thresholds." },
  ...
  { query: /\b(?:eps|earnings)\s+growth\b|\bpositive earnings growth\b|\b(?:eps|earnings)(?:\s+growth)?\s+double[- ]digits?\b/i, coveredBy: ["epsGrowth3Y"], assumptionMentions: /earnings growth|eps growth|growth horizon/i, message: "EPS/earnings growth needs a time horizon; Parse supports 3-year EPS growth, so this unqualified criterion was left out." },
  { query: /\bdividend growers?\b/i, coveredBy: ["divGrowth5Y"], assumptionMentions: /dividend grow/i, message: "'Dividend grower' needs a growth horizon and threshold; Parse supports 5-year dividend growth when those are specified, so no threshold was guessed." },
