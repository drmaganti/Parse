...
const INTENT_COVERAGE: IntentCoverageSpec[] = [
  ...
  { query: /\blow debt\b|\blittle debt\b|\bminimal debt\b|\blow leverage\b|\blightly leveraged\b|\bleverage\s+(?:is|looks?)\s+low\b/i, coveredBy: ["debtEquity"], assumptionMentions: /low debt|little debt|minimal debt|low leverage|lightly leveraged|leverage\s+(?:is|looks?)\s+low|debt\s*\/?\s*equity/i, message: "'Low debt/leverage' needs a debt/equity threshold; Parse left that criterion out rather than guess." },
